...
WHERE inventory.item_type = 'consumable' 
  AND inventory.locate NOT IN ('BSIT', 'BEED', 'BSHM', 'HCS', 'SHS', 'BSBA', 'BSCRIM')
...
<div class="btn-toolbar mb-3" role="toolbar" aria-label="Inventory Categories" style="justify-content: center;">
    <div class="btn-group" role="group" aria-label="Inventory Categories">
        ...
        <a href="reports_unspecified.php" class="btn btn-primary">Unspecified Inventory</a>
    </div>
</div>
    </br>
...
<script>
    function printAndRedirect() {
        window.location.href = 'text_file.php?what=unspecified';
    }
</script>
...
<div class="card">
    ...
    <?php
    $shs_query = mysqli_query($mysqli, "SELECT inventory.item_code AS barcode_id, 
    ...
    WHERE inventory.locate NOT IN ('BSIT', 'BEED', 'BSHM', 'HCS', 'SHS', 'BSBA', 'BSCRIM') 
    GROUP BY inventory.item_code");
    if ($shs_query) {
        if(mysqli_num_rows($shs_query) > 0) {
            while ($shs_row = mysqli_fetch_array($shs_query)) {
                echo "<tr style='border: 1px solid black;'>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['barcode_id'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['equipment_name'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['equipment_brand'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['available_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['borrowed_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $shs_row['item_type'] . "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>No Unspecified inventory data found.</td></tr>";
        }
    } else {
        echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>Error fetching SHS inventory data: " . mysqli_error($mysqli) . "</td></tr>";
    }
    ?>
</table>
