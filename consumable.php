...
$rt = mysqli_query($mysqli, "SELECT * FROM archive_inventory where item_type = 'consumable'");
...
if (isset($_POST['search_barcode'])) {
    ...
    $query = "SELECT inventory.*, COALESCE(SUM(borrowed_item.borrowed_amount), 0) AS total_borrowed_amount 
              ...
              AND inventory.item_type = 'consumable'
...
$total_pages = ceil(mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM inventory WHERE item_type = 'consumable'")) / $limit);
...
<body>
    ...
    <div class="sidebar p-1">
        ...
        <a class="me-1 btn btn-primary" onclick="add()">Add Consumable</a>
<script>
    function add() {
        var modalContent = `
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="addProductModalLabel">Add Consumable</h5>
                                        ...
                                        <tbody>
                                            <?php
                                                while($ret=mysqli_fetch_array($rt)) {
                                                    ...
                                                    echo "<td><a style='height:50%' class='btn btn-sm btn-danger' href='sunarchive_item.php?id=".$ret['id']."&type=consumable'>Restore</a>&nbsp;&nbsp;<a style='height:50%' class='btn btn-sm btn-danger' href='spermanent_delete.php?id=".$ret['id']."&type=consumable'>Remove</a></td>";
                                                    echo "</tr>";
                                                }
                                            ?>
                                        </tbody>
