...
if(isset($_POST['search_btn'])) {
    ...
    $query = "SELECT * FROM archive_user WHERE (username LIKE '%$searchTerm%' OR password LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' OR department LIKE '%$searchTerm%' OR position LIKE '%$searchTerm%' OR iid LIKE '%$searchTerm%') AND user_type LIKE 'supply_user'";
    $rs = mysqli_query($mysqli, $query);
} elseif(isset($_POST['search_reset'])) {
    $searchTerm = "";
    $rs = mysqli_query($mysqli, "SELECT * FROM archive_user WHERE user_type = 'supply_user'");
} else {
    $rs = mysqli_query($mysqli, "SELECT * FROM archive_user WHERE user_type = 'supply_user'");
}
?>
...
if (mysqli_num_rows($rs) > 0) {
    while ($res = mysqli_fetch_array($rs)) {
        $img = empty($res['image_path']) ? "inventory/not.jpg" : $res['image_path'];
        echo "<tr>";
        echo "<td><img class='myImg' src='" . $img . "' alt='" . $res['username'] . "' style='width: 40px; height: 40px; border-radius: 50%; border: 2px solid #000;'></td>";
        echo "<td align='center'>" . $res['username'] . "</td>";
        echo "<td align='center'>" . $res['email'] . "</td>";
        echo "<td align='center'>" . $res['department'] . "</td>";
        echo "<td align='center'>" . $res['position'] . "</td>";
        echo "<td align='center'>" . $res['iid'] . "</td>";
            echo "<td align='center'>
                    <a class='btn btn-sm btn-danger' href='#' onclick='unarchiveItemConfirmation({$res['eid']});'>Unarchive User</a> 
                    <a class='btn btn-sm btn-danger' href='#' onclick='permanentDeleteConfirmation({$res['eid']});'>Permanently Delete</a>
                  </td>";
            echo "</tr>";
    }
} else {
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="itemDetails">
                    <p><strong>Username: </strong><span id="item_code"></span></p>
                    <p><strong>Email: </strong><span id="equipment_name"></span></p>
                    <p><strong>Departmnet: </strong><span id="equipment_brand"></span></p>
                    <p><strong>Position: </strong><span id="quantity"></span></p>
                    <p><strong>User ID: </strong><span id="item_type"></span></p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function unarchiveItemConfirmation(itemId) {
        ...
        document.getElementById('confirmUnarchiveBtn').addEventListener('click', function() {
            window.location.href = 'unarchive_user.php?id=' + itemId + '&type=supply';
        });
    }
    ...
    function permanentDeleteConfirmation(itemId) {
        ...
        document.getElementById('confirmPermanentDeleteBtn').addEventListener('click', function() {
            window.location.href = 'permanent_delete_user.php?id=' + itemId + '&type=supply';
        });
    }
</script>
</body>
