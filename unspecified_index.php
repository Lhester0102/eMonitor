...
if(isset($_POST['search_btn'])) {
    ...
    $query = "SELECT inventory.*, COALESCE(SUM(borrowed_item.borrowed_amount), 0) AS total_borrowed_amount 
              ...
              WHERE (inventory.id LIKE '%$searchTerm%' OR inventory.item_code LIKE '%$searchTerm%' OR inventory.equipment_name LIKE '%$searchTerm%' OR inventory.equipment_brand LIKE '%$searchTerm%' OR inventory.quantity LIKE '%$searchTerm%') AND inventory.locate NOT IN ('BSIT', 'BEED', 'BSHM', 'HCS', 'SHS', 'BSBA', 'BSCRIM')
...
if (isset($_POST['switch'])) {
    if ($_SESSION['nom'] === 0) {
        $query = "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
              FROM inventory 
              LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
              WHERE inventory.item_type = 'non-consumable' AND inventory.locate NOT IN ('BSIT', 'BEED', 'BSHM', 'HCS', 'SHS', 'BSBA', 'BSCRIM')
              GROUP BY inventory.id";
        $rs = mysqli_query($mysqli, $query);
        $_SESSION['nom'] = 1;
        ...
        $query = "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
              FROM inventory 
              LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
              WHERE inventory.item_type = 'consumable' AND inventory.locate NOT IN ('BSIT', 'BEED', 'BSHM', 'HCS', 'SHS', 'BSBA', 'BSCRIM')
              GROUP BY inventory.id";
        $rs = mysqli_query($mysqli, $query);
        $_SESSION['nom'] = 2;
    } else {
...
<div class="container mt-4">
    ...
    <a href="unspecified_index.php" class="btn btn-primary">Unspecified Inventory</a>
</div>
    ...
    <tbody>
        ...
        if ($rs) {
            ...
            echo "<tr><td colspan='8' class='text-center'>No item found.</td></tr>";
        }
    } else {
        // Display error message if query fails
        echo "<tr><td colspan='9' class='text-center'>Error fetching inventory data: " . mysqli_error($mysqli) . "</td></tr>";
    }
