...
$how=$_REQUEST['how'];
$itemCode="";
$rs=mysqli_query($mysqli,"select barcode from merch where mid=$id");
...
if ($uploadedFileName != $code . '.' . $imageFileType) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["barcode_img"]["tmp_name"], $targetFile)) {
            ...
            $updateQuery = "UPDATE merch SET barcode='$code' WHERE mid=$id";
...
<div class="sidebar" <?php if ($how == 1) { echo "hidden"; } else { } ?>>
    <a href="generator.php?id=<?php echo $id; ?>&code=<?php echo $code; ?>" >Back</a>
</div>
