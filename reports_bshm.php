...
$rs = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          ...
          WHERE inventory.item_type = 'consumable' AND inventory.locate = 'BSHM'
          GROUP BY inventory.id");

$rt = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          FROM inventory 
          LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
          WHERE inventory.item_type = 'non-consumable' AND inventory.locate = 'BSHM'
          GROUP BY inventory.id");
...
<div class="btn-toolbar mb-3" role="toolbar" aria-label="Inventory Categories" style="justify-content: center;">
    <div class="btn-group" role="group" aria-label="Inventory Categories">
        ...
        <a href="reports_bshm.php" class="btn btn-primary">BSHM Inventory</a>
...
<script>
    function printAndRedirect() {
        window.location.href = 'text_file.php?what=BSHM';
    }
</script>
...
<div class="card">
        <div class="card-body">
            <div class="table-responsive">  
                ...
                <thead>
                        <tr> 
    <tr> <th colspan="6"><b><center>BSHM Inventory</b></th> </tr></center>
    ...
    <?php
    $bshm_query = mysqli_query($mysqli, "SELECT inventory.item_code AS barcode_id, 
    ...
    WHERE inventory.locate = 'BSHM' 
    GROUP BY inventory.item_code");
    if ($bshm_query) {
        if(mysqli_num_rows($bshm_query) > 0) {
            while ($bshm_row = mysqli_fetch_array($bshm_query)) {
                echo "<tr style='border: 1px solid black;'>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['barcode_id'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['equipment_name'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['equipment_brand'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['available_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['borrowed_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $bshm_row['item_type'] . "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>No BSHM inventory data found.</td></tr>";
        }
    } else {
        echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>Error fetching BSHM inventory data: " . mysqli_error($mysqli) . "</td></tr>";
    }
    ?>
</table>
