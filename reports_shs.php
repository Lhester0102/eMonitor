...
$rs = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          ...
          WHERE inventory.item_type = 'consumable' AND inventory.locate = 'SHS'
          GROUP BY inventory.id");

$rt = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          FROM inventory 
          LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
          WHERE inventory.item_type = 'non-consumable' AND inventory.locate = 'SHS'
          GROUP BY inventory.id");
...
<div class="btn-toolbar mb-3" role="toolbar" aria-label="Inventory Categories" style="justify-content: center;">
    <div class="btn-group" role="group" aria-label="Inventory Categories">
        ...
        <a href="reports_shs.php" class="btn btn-primary">SHS Inventory</a>
...
<script>
    function printAndRedirect() {
        window.location.href = 'text_file.php?what=SHS';
    }
</script>
...
<div class="card">
    ...
    if ($shs_query) {
        if(mysqli_num_rows($shs_query) > 0) {
            ...
            echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>No SHS inventory data found.</td></tr>";
        }
    } else {
