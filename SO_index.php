...
if(isset($_POST['search_btn'])) {
    ...
    $query = "SELECT inventory.*, COALESCE(SUM(borrowed_item.borrowed_amount), 0) AS total_borrowed_amount 
              ...
              WHERE (inventory.id LIKE '%$searchTerm%' OR inventory.item_code LIKE '%$searchTerm%' OR inventory.equipment_name LIKE '%$searchTerm%' OR inventory.equipment_brand LIKE '%$searchTerm%' OR inventory.quantity LIKE '%$searchTerm%') AND inventory.locate = 'Supply Officer'
    ...
    $rs = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          ...
          WHERE inventory.locate = 'Supply Officer'
          GROUP BY inventory.id");
}
...
if (isset($_POST['switch'])) {
    if ($_SESSION['nom'] === 0) {
        $query = "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
              FROM inventory 
              LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
              WHERE inventory.item_type = 'non-consumable' AND inventory.locate = 'Supply Officer'
              GROUP BY inventory.id";
        $rs = mysqli_query($mysqli, $query);
        $_SESSION['nom'] = 1;
        ...
        $query = "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
              FROM inventory 
              LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
              WHERE inventory.item_type = 'consumable' AND inventory.locate = 'Supply Officer'
              GROUP BY inventory.id";
        $rs = mysqli_query($mysqli, $query);
        $_SESSION['nom'] = 2;
    } else {
...
<div class="container mt-4">
    <div class="card">
        <div class="card-header">
            <h3 class="text-center">ADMIN Inventory</h3>
        </div>
        <br>
    ...
    <a href="SO_index.php" class="btn btn-primary">Admin Inventory</a>
