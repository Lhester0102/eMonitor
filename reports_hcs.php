...
$rs = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          ...
          WHERE inventory.item_type = 'consumable' AND inventory.locate = 'HCS'
          GROUP BY inventory.id");

$rt = mysqli_query($mysqli, "SELECT inventory.*, COALESCE(SUM(borrowed_amount), 0) AS total_borrowed_amount 
          FROM inventory 
          LEFT JOIN borrowed_item ON inventory.id = borrowed_item.item_code 
          WHERE inventory.item_type = 'non-consumable' AND inventory.locate = 'HCS'
          GROUP BY inventory.id");
...
<div class="btn-toolbar mb-3" role="toolbar" aria-label="Inventory Categories" style="justify-content: center;">
    <div class="btn-group" role="group" aria-label="Inventory Categories">
        ...
        <a href="reports_hcs.php" class="btn btn-primary">HCS Inventory</a>
...
<script>
    function printAndRedirect() {
        window.location.href = 'text_file.php?what=HCS';
    }
</script>
...
<div class="card">
    ...
    <?php
    $hcs_query = mysqli_query($mysqli, "SELECT inventory.item_code AS barcode_id, 
    ...
    WHERE inventory.locate = 'HCS' 
    GROUP BY inventory.item_code");
    if ($hcs_query) {
        if(mysqli_num_rows($hcs_query) > 0) {
            while ($hcs_row = mysqli_fetch_array($hcs_query)) {
                echo "<tr style='border: 1px solid black;'>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['barcode_id'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['equipment_name'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['equipment_brand'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['available_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['borrowed_number'] . "</td>";
                echo "<td style='border: 1px solid black;'>" . $hcs_row['item_type'] . "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>No HCS inventory data found.</td></tr>";
        }
    } else {
        echo "<tr style='border: 1px solid black;'><td colspan='6' class='text-center'>Error fetching HCS inventory data: " . mysqli_error($mysqli) . "</td></tr>";
    }
    ?>
</table>
